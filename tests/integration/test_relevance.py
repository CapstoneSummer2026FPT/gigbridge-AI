...
from scripts.answer import answer_question as cli_answer_question, RelevanceCheck as CLIRelevanceCheck
...
def test_cli_unrelated_query_vietnamese():
    ...
    with patch("scripts.answer.check_relevance", return_value=mock_relevance):
...
async def test_service_unrelated_query_vietnamese():
    ...
    with patch.object(service.query_engine, "check_relevance", AsyncMock(return_value=mock_relevance)):
        config = AnswerConfig()
...
async def test_service_on_topic_query_proceeds():
    ...
    with patch.object(service.query_engine, "check_relevance", AsyncMock(return_value=mock_relevance)):
        with patch.object(service.retriever, "fetch_context", AsyncMock(return_value=[])):
            ...
            with patch("app.services.rag.query_engine.acompletion", AsyncMock(return_value=mock_resp)):
                config = AnswerConfig()
