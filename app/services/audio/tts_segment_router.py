"""
PURPOSE: Bilingual TTS segment routing for mixing English technical terms with Vietnamese speech text.
IMPORTANCE: High — Splits sentences into language-specific chunks for multi-voice bilingual synthesis.
READING FLOW: app/services/audio/tts_segment_router.py -> app/clients/voice/gateway.py
"""
...
class TTSSegment:
    """Dataclass storing text segment and target synthesis language."""
    text: str
    language: str
...
class TTSSegmentRouter:
    ...
    def route(
        ...
        """Route text into language-tagged TTSSegment objects.
        
        Flow:
        1. Normalize base language ('vi' vs 'en').
        2. Filter English technical terms from hotwords list.
        3. Split text into sentence boundary strings.
        4. Detect language per sentence and route English hotwords to 'en'.
        5. Merge adjacent segments sharing the same language.
        """
        if not text:
            return []
    ...
    def _route_sentence(
        ...
        """Route hotwords within a sentence."""
    ...
    def _detect_sentence_language(self, sentence: str, base_language: str) -> str:
        """Detect language of single sentence using diacritics and vocabulary."""
    ...
    def _english_hotwords(self, hotwords: list[str]) -> list[str]:
        """Filter English terms from hotwords list."""
        seen = set()
        terms = []
    ...
    def _looks_english_term(self, term: str) -> bool:
        """Check if term looks like an English technical term."""
    ...
    def _find_hotword_matches(sentence: str, terms: list[str]) -> list[tuple[int, int]]:
        """Find regex spans of terms in sentence."""
    ...
    def _merge_adjacent(segments: list[TTSSegment]) -> list[TTSSegment]:
        """Merge adjacent TTSSegment items sharing identical language."""
