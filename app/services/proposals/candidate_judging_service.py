...
class CandidateJudgingService:
    ...
    async def evaluate_candidate(
        ...
        system_prompt = (
            ...
            "STRICT PROPOSAL LANGUAGE CORRESPONDENCE RULE:\n"
            "- CRITICAL: Output ALL evaluation text fields (`requirement_fulfillment` items including `requirement` and `evidence_quote`, `screening_qa` feedback, `pillar_comments`, `answer_quality_summary_comment`, and `probing_questions`) strictly in the PRIMARY LANGUAGE of the candidate proposal.\n"
            "- If the candidate proposal text is written in VIETNAMESE, output ALL evaluation text fields in clear, professional VIETNAMESE.\n"
            "- If the candidate proposal text is written in ENGLISH, output ALL evaluation text fields in clear, professional ENGLISH. If the job description requirements are in Vietnamese, translate the `requirement` name into English to ensure full language alignment with the candidate proposal.\n\n"
