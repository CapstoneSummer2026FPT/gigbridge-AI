"""
PURPOSE: Evidence evaluation service for checking factual truthfulness and claims against source context.
IMPORTANCE: High — Provides evidence fact-checking and claim verification for AI benchmarks and diagnostics.
READING FLOW: app/schemas/eval_schemas.py -> app/services/rag/evaluator.py -> app/api/routes/evaluation_api.py
"""
...
class EvidenceEvaluatorService:
    ...
    def __init__(self, model_name: str | None = None):
        """Initialize EvidenceEvaluatorService with target LLM model."""
    ...
    def evaluate(self, source_context: str, candidate_evidence: str) -> EvidenceEvalResponse:
        ...
        Flow:
        1. Check for empty source context or candidate evidence.
        2. Format fact-checking evaluation prompt.
        3. Invoke LLM completion with fallback handling.
        4. Parse returned claim JSON objects.
        5. Compute overall truthfulness percentage and generate sentence-level color-coded HTML.
        """
    ...
    def _build_response(self, candidate_evidence: str, claims: List[ClaimDetail]) -> EvidenceEvalResponse:
        """Calculate truth score and build annotated HTML visualization."""
        if not claims:
