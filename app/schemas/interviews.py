"""
PURPOSE: Pydantic schemas for stateful voice & text candidate interview screening and evaluation.
IMPORTANCE: Critical — Defines request and response structures for interview lifecycle, STT drafts, TTS audio, and candidate feedback.
READING FLOW: app/schemas/interviews.py -> app/services/interviews/interview_base.py -> app/services/interviews/* -> app/api/routes/interviews.py
"""
...
class ConfirmAnswerRequest(BaseModel):
    ...
    def reject_blank_corrected_text(cls, value: Optional[str]) -> Optional[str]:
        """Validate that corrected_text is non-empty if provided."""
        if value is None:
            return None
        value = value.strip()
        if not value:
